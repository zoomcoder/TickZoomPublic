...
{
    public abstract class LimeQuoteProviderSupport : AgentPerformer, LogAware
    {
        public static ushort QuotesSimulatorPort = 7047;
        ...
        public LimeQuoteProviderSupport(string name)
        {
            configSection = name;
            log = Factory.SysLog.GetLogger(typeof(LimeQuoteProviderSupport) + "." + GetType().Name);
            log.Register(this);
        ...
        protected void OnDisconnect(Socket socket)
        {
            ...
            {
                log.Error("LimeQuoteProvider disconnected.");
                CreateNewSocket();
            }
        }
            ...
            switch (socket.State)
            {
				...
				case SocketState.PendingConnect:
					if( Factory.Parallel.TickCount >= retryTimeout) {
                        log.Info("LimeQuoteProvider connect timed out. Retrying.");
						SetupRetry();
	    ...
	    private Yield TrySetupRetry()
	    {
	        switch( ConnectionStatus) {
	            ...
	            case Status.PendingRetry:
	                if( Factory.Parallel.TickCount >= retryTimeout) {
	                    log.Info("LimeQuoteProvider retry time elapsed. Retrying.");
	                    OnRetry();
	                    CreateNewSocket();
	                    if( trace) log.Trace("ConnectionStatus changed to: " + ConnectionStatus);
	                    return Yield.DidWork.Repeat;
	    ...
	    private void LoadProperties(string configFilePath) {
            ...
            configFile.AssureValue("EquityDemo/ServerAddress", "74.120.51.3");
            configFile.AssureValue("EquityDemo/ServerPort", "7047");
            configFile.AssureValue("EquityDemo/UserName", "CHANGEME");
            configFile.AssureValue("EquityDemo/Password", "CHANGEME");
            configFile.AssureValue("EquityDemo/QuoteServiceID", "BATS");

            configFile.AssureValue("EquityLive/UseLocalTickTime", "true");
            configFile.AssureValue("EquityLive/ServerAddress", "74.120.51.24");
            configFile.AssureValue("EquityLive/ServerPort", "CHANGEME");
            configFile.AssureValue("EquityLive/UserName", "CHANGEME");
            configFile.AssureValue("EquityLive/Password", "CHANGEME");
            configFile.AssureValue("EquityLive/QuoteServiceID", "CHANGEME");
        ...
        private void Exception( string field, ConfigFile configFile) {
            ...
            sb.AppendLine("For example, LimeFIXProvider/EquityDemo will choose the LimeFIXProvider.exe assembly");
            sb.AppendLine("with the EquityDemo section within the LimeFIXProvider\\Default.config file for that assembly.");
        ...
        public LimeQuoteProviderSupport.Status ConnectionStatus
        {
