...
//			File.Delete( Factory.SysLog.LogFolder + @"\" + assemblyName+".log");			
		...
		private void ClearPositionInternal(Agent provider, VerifyFeed verify, int expectedPosition) {
		    ...
		    var strategyPositions = new ActiveList<StrategyPosition>();
  			provider.SendEvent(new EventItem(verify.Task,symbol,EventType.PositionChange,new PositionChangeDetail(symbol,expectedPosition,orders,strategyPositions,TimeStamp.UtcNow.Internal,1L)));
		}
...
//		}
		...
		public void SendOrders(Agent provider, VerifyFeed verify, int desiredPosition, int secondsDelay) {
  			if( SyncTicks.Enabled) tickSync.AddPositionChange("Test");
            var strategyPositions = new ActiveList<StrategyPosition>();
            provider.SendEvent(new EventItem(verify.Task, symbol, EventType.PositionChange, new PositionChangeDetail(symbol, desiredPosition, orders, strategyPositions, TimeStamp.UtcNow.Internal, 1L)));
		}
