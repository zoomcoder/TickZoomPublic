...
//			File.Delete( Factory.SysLog.LogFolder + @"\" + assemblyName+".log");			
		...
		private void ClearPositionInternal(Provider provider, VerifyFeed verify, int expectedPosition) {
		    ...
		    var strategyPositions = new ActiveList<StrategyPosition>();
  			provider.SendEvent(new EventItem(verify,symbol,(int)EventType.PositionChange,new PositionChangeDetail(symbol,expectedPosition,orders,strategyPositions,TimeStamp.UtcNow.Internal,1L)));
		}
...
//		}
		...
		public void SendOrders(Provider provider, VerifyFeed verify, int desiredPosition, int secondsDelay) {
  			if( SyncTicks.Enabled) tickSync.AddPositionChange("Test");
            var strategyPositions = new ActiveList<StrategyPosition>();
            provider.SendEvent(new EventItem(verify, symbol, (int)EventType.PositionChange, new PositionChangeDetail(symbol, desiredPosition, orders, strategyPositions, TimeStamp.UtcNow.Internal,1L)));
		}
